...
----------------------------------------------------------------------------------------------------------------------*/
// It is a manager for a ResourceGrid subcomponent, which does most of the heavy lifting.
// It is responsible for managing width/height.

var ResourceView = $.fullCalendar.View.extend({

	resourceGrid: null, // the main subcomponent that does most of the heavy lifting
	...
	initialize: function() {
		this.resourceGrid = new $.fullCalendar.ResourceGrid(this);
	...
	setRange: function(range) {
		$.fullCalendar.View.prototype.setRange.call(this, range); // call the super-method
	...
	computeRange: function(date) {
		var range = $.fullCalendar.View.prototype.computeRange.call(this, date); // get value from the super-method
	...
	renderDates: function() {
		...
		this.resourceGrid.numbersVisible = this.dayNumbersVisible || this.weekNumbersVisible;
		...
		this.resourceGrid.setElement(this.el.find('.fc-day-grid'));
				...
				'<thead class="fc-head">' +
					'<tr>' +
						'<td class="' + this.widgetHeaderClass + '">' +
							this.resourceGrid.headHtml() + // render the day-of-week headers
						'</td>' +
					'</tr>' +
				'</thead>' +
				'<tbody class="fc-body">' +
	...
	// Queried by the ResourceGrid subcomponent when generating rows. Ordering depends on isRTL.
	...
	// Queried by the ResourceGrid subcomponent. Ordering depends on isRTL.
	numberIntroHtml: function(row) {
		if (this.weekNumbersVisible) {
			return '' +
				'<td class="fc-week-number" ' + this.weekNumberStyleAttr() + '>' +
					'<span>' + // needed for matchCellWidths
						this.resourceGrid.getCell(row, 0).start.format('w') +
					'</span>' +
				'</td>';
		}
	},
	...
	// Generates the HTML that goes before every other type of row generated by ResourceGrid. Ordering depends on isRTL.
	...
	// Generates the HTML for the <td>s of the "number" row in the ResourceGrid's content skeleton.
	...
	numberCellHtml: function(cell) {
		...
		classes = this.resourceGrid.getDayClasses(date);
	...
	renderEvents: function(events) {
		this.resourceGrid.renderEvents(events);
	...
	renderSelection: function(range) {
		this.resourceGrid.renderSelection(range);
	},
